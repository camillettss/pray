...
from core.util import sign, timer

class Game():
    def __init__(self, world, generator, offsets) -> None:
        ...
        # mobs and items
        self.setup_objects_and_entities()

    def setup_objects_and_entities(self):
        return
        
    def _gravity(self):
        if not self.get_block(self.player.x, self.player.y-0.5, self.player.z-self.motion_amount):
            self.player.y -= self.motion_amount-(self.motion_amount//2)
    ...
    def update(self,dt):
        ...
        if self.keys[key.SPACE]:
            if self.get_block(self.player.x, self.player.y-0.5, self.player.z):
                if self.clockdone:
                    self.clockdone=False
                    for _ in range(10):
                        self.player.y += 0.05*5
                    _thread.start_new_thread(timer, (0.5, self))

        '''elif self.keys[key.LCTRL]:
            if not self.get_block(self.player.x, self.player.y-self.motion_amount, self.player.z):
                self.player.y -= self.motion_amount'''
        if self.keys[key.LSHIFT]:
            #print(dt)
            self.motion_amount+=0.075
        else:
            self.motion_amount=dt*5
        ...
        #if True:#not self.get_block(self.player.x, self.player.y, self.player.z):
        '''if w&a:
            self.player.apply_motion(self,self.motion_amount,45)
        elif w&d:
            self.player.apply_motion(self,self.motion_amount,-45)
        elif s&d:
            self.player.apply_motion(self,self.motion_amount,-0.07535)
        elif s&a:
        ...
        if w&a:
            if not self.get_block(self.player.x-sin(radians(self.player.angle_y+45)) * self.motion_amount, self.player.y, self.player.z-cos(radians(self.player.angle_y+45)) * self.motion_amount):
                self.player.apply_motion(self,self.motion_amount,45)
        elif w&d:
            if not self.get_block(self.player.x-sin(radians(self.player.angle_y-45)) * self.motion_amount, self.player.y, self.player.z-cos(radians(self.player.angle_y-45)) * self.motion_amount):
                self.player.apply_motion(self,self.motion_amount,-45)
        elif s&d:
            if not self.get_block(self.player.x-sin(radians(self.player.angle_y-0.07535)) * self.motion_amount, self.player.y, self.player.z-cos(radians(self.player.angle_y-0.07535)) * self.motion_amount):
                self.player.apply_motion(self,self.motion_amount,-0.07535)
        elif s&a:
            if not self.get_block(self.player.x-sin(radians(self.player.angle_y+135)) * self.motion_amount, self.player.y, self.player.z-cos(radians(self.player.angle_y+135)) * self.motion_amount):
                self.player.apply_motion(self,self.motion_amount,135)
        elif w:
            if not self.get_block(self.player.x-sin(radians(self.player.angle_y+0)) * self.motion_amount, self.player.y, self.player.z-cos(radians(self.player.angle_y+0)) * self.motion_amount):
                self.player.apply_motion(self,self.motion_amount)
        elif s:
            if not self.get_block(self.player.x-sin(radians(self.player.angle_y+180)) * self.motion_amount, self.player.y, self.player.z-cos(radians(self.player.angle_y+180)) * self.motion_amount):
                self.player.apply_motion(self,self.motion_amount,180)
        elif a:
            if not self.get_block(self.player.x-sin(radians(self.player.angle_y+90)) * self.motion_amount, self.player.y, self.player.z-cos(radians(self.player.angle_y+90)) * self.motion_amount):
                self.player.apply_motion(self,self.motion_amount,90)
        elif d:
            if not self.get_block(self.player.x-sin(radians(self.player.angle_y-90)) * self.motion_amount, self.player.y, self.player.z-cos(radians(self.player.angle_y-90)) * self.motion_amount):
                self.player.apply_motion(self,self.motion_amount,-90)
        ...
        to_check = list(self.key_listing).pop()
    ...
    def on_mouse_motion(self,x,y,dx,dy):
        if not self.inventory:
            self.player.angle_y -= dx *0.2 #NOTE: These are reserved.
            self.player.angle_x += dy *0.2
    ...
    def on_mouse_press(self,x,y,button,modifiers):
        if button == pyglet.window.mouse.LEFT:
            ...
            b = self.cast_to_side()
            if b is not None:
                self.set_block(b[0],b[1],b[2],blocks.Forge(b[0],b[1],b[2]))
            '''if self.inventory:
                x,y = self.invent_pos
                self.inventory = False
